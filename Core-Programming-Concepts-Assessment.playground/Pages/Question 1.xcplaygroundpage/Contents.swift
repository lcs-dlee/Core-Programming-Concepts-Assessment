/*:
 [Previous](@previous)
 ### Learning Goal
 
 * Software Development: Goal 8
    * *Communication*
        * I can write human-readable programs (naming, indenting) with appropriate documentation (comments).

 # Question 1

 The code below translates a temperature measured in degrees Celsius to degrees Fahrenheit.
 
 To complete this question:
 
 * where necessary, **change the constant names so they are more descriptive of what they keep track of**
 * revise all constant names to use the **camel case convention**
 
 Remember, when writing code in Swift, aim for **clarity**, not brevity.
 
 */
import Foundation

// Revise code below to answer question 1
let temperatureInDegreesCelsius : Double = 20
let temperatureInFahrenheit = temperatureInDegreesCelsius * 9 / 5 + 32
print("A temperature in degrees Celsius of \(temperatureInDegreesCelsius) is equal to \(temperatureInFahrenheit) in degrees Fahrenheit.")

/*:
 - callout(Shortcut): The **Edit All in Scope** menu command is useful when renaming variables – place your cursor within the variable name and then invoke the menu command.
 
 ![edit_all_in_scope](editallinscope.png "Edit All in Scope")
 
 ## Now share your understanding
 
 1. Commit your response on this page (Option-Command-C).
 2. [Add a link][al] to your Computer Science portfolio.
 
 [al]:
 https://www.youtube.com/watch?v=Wa3Wl3T25jo&list=PLTGGOQnktyWs9TlNJ30pgYgypvIGrt3Lx&index=1
 
 ### Learning Goal
 
 * Software Development: Goal 8
    * *Communication*
        * I can write human-readable programs (naming, indenting) with appropriate documentation (comments).

 [Next](@next)
 */
